...
public class TileMapEditor : Editor {
	...
	void UpdateCalculations(){
	    ...
	    if (map.spriteReferences.Length < 1)
	        return;
	...
	void MoveBrush(){
		...
		var id = coordToId((int)row, (int)column, (int)map.mapSize.x);
    ...
    int coordToId(int row, int column, int columnsPerRow)
    {
        return (column* columnsPerRow) + row;
    }

    Vector2 idToCoord(int id, int columnsPerRow)
    {
        int row = id/columnsPerRow;
        int column = id%columnsPerRow;
        return new Vector2(column, row);
    }

	void Draw(){
		var coord = idToCoord(brush.tileID, (int)map.mapSize.x);

		var posX = brush.transform.position.x;
		var posY = brush.transform.position.y;

        var id = "tile_r" + coord.y + "_c" + coord.x;
        GameObject tile = GameObject.Find (map.name + "/Tiles/" + id);
		if (tile == null) {
			tile = new GameObject(id);
